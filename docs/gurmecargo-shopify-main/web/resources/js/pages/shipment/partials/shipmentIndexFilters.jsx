...
export default function ShipmentIndexFilters({
    setShipments,
    shipments,
    appName,
}) {
    ...
    const handleSortChange = (value) => {
        ...
        const sortedShipments = [...shipments].sort((a, b) => {
            const dateA = new Date(a.shipment.created_at);
            const dateB = new Date(b.shipment.created_at);
            if (value[0] === "date asc") {
                return dateA - dateB;
    ...
    const tabs = itemStrings.map((item, index) => ({
        ...
        onAction: () => {
            if (item === "Hepsi") {
                open(`shopify://admin/apps/${appName}/shipment`, "_self")
            } else {
                open(`shopify://admin/apps/${appName}/shipment?status=${reverseStatusMapping[item]}`, "_self")
            }
        },
        id: `${item}-${index}`,
        isLocked: index === 0,
